...
public class UsersCreateIntegrationStepDefs {
    ...
    @Value("${password_pepper:NOT_LOADED}")
    private String vaultPepper;
    ...
    public void thePasswordHashIncludesTheVaultPepper() {
        assertThat(vaultPepper)
                .as("Pepper should be loaded from Vault")
                .isNotNull()
                .isNotEmpty()
                .isNotEqualTo("NOT_LOADED")
                .isNotEqualTo("test-pepper-value-for-unit-tests");
        
        // Verify pepper is the expected Vault value (at least check it's not the test mock)
        assertThat(vaultPepper.length())
                .as("Vault pepper should be substantial length (not a simple test value)")
                .isGreaterThan(20);
    }

    @And("the password hash is different from test environment hash")
    public void thePasswordHashIsDifferentFromTestEnvironmentHash() throws Exception {
        // Since we use a different pepper than the test mock, hashes will differ
        // This verifies we're actually using Vault's pepper and not the test mock
        assertThat(vaultPepper)
                .as("Should use real Vault pepper, not test mock")
                .isNotEqualTo("test-pepper-value-for-unit-tests");
    }
}
